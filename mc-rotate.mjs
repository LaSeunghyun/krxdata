/**
 * mc-rotate.mjs — 로테이션 MC + **노이즈 바닥 동시 측정** (2026-07-30)
 *
 * ═══ 왜 방식을 바꾸나 ═══
...
const CONFIGS = [
  // ── 규칙 축 (데이터 동일, 규칙만 다름) ──
  { key: 'off', name: 'off (로테이션 없음)', file: T, extra: '', grp: 'rule' },
  { key: 'r03', name: '보수 손실만·3일+ (0/3)', file: T, extra: '--rotate --rotmaxret 0 --rotminhold 3', grp: 'rule' },
  { key: 'r01', name: '손실만·1일차부터 (0/1)', file: T, extra: '--rotate --rotmaxret 0 --rotminhold 1', grp: 'rule' },
  { key: 'r21', name: '이익+2%까지·1일차 (2/1) ★사용자안', file: T, extra: '--rotate --rotmaxret 2 --rotminhold 1', grp: 'rule' },
  { key: 'r23', name: '이익+2%까지·3일+ (2/3)', file: T, extra: '--rotate --rotmaxret 2 --rotminhold 3', grp: 'rule' },
  { key: 'r05', name: '보수 손실만·5일+ (0/5)', file: T, extra: '--rotate --rotmaxret 0 --rotminhold 5', grp: 'rule' },
...
console.log(`=== 로테이션 ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
...
if (!real.length) console.log(`실질 효과 0건 — 모든 로테이션 변종이 노이즈 바닥(${NOISE.toFixed(2)}) 안에 있다.`);
