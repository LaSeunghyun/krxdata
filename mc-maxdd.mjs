...
const CONFIGS = [
  // ── 20일 낙폭 상한 필터. "2일 -25% 급락은 과매도가 아니라 붕괴이므로 사지 않는다" ──
  //   실측 동기(2026-07-30 라이브): rsi2 가 산 5종목의 20일 낙폭 —
  //     LG이노텍 -53.3%(당일 -7.5%) · 삼성전기 -52.9%(당일 -12.6%) · 두산 -32.8%(-9.2%)
  //     · HD현대마린솔루션 -19.3%(-2.4%) · 한국타이어 +6.5%(-5.4%)
  //   RSI2 는 방향만 보고 **크기를 안 본다** — 2일 -1% 와 2일 -25% 가 똑같이 0이다.
  //   ※ 오늘 기각된 축들과 다르다: rsimindist/MA정렬/ATR정렬은 **상대 순위**였고 이건 **절대 상한**이다.
  { key: 'd0', name: 'off (필터 없음, 현행)', file: T, extra: '', grp: 'rule' },
  { key: 'd50', name: '20일낙폭 -50% 초과 제외', file: T, extra: '--rsimaxdd20 50', grp: 'rule' },
  { key: 'd40', name: '20일낙폭 -40% 초과 제외', file: T, extra: '--rsimaxdd20 40', grp: 'rule' },
  { key: 'd30', name: '20일낙폭 -30% 초과 제외', file: T, extra: '--rsimaxdd20 30', grp: 'rule' },
  { key: 'd20', name: '20일낙폭 -20% 초과 제외', file: T, extra: '--rsimaxdd20 20', grp: 'rule' },
...
console.log(`=== 20일 낙폭 상한 필터 ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
...
console.log(`\n=== 판정 ===`);
const rules = R.filter(r => r.grp === 'rule' && r.key !== 'd0');
