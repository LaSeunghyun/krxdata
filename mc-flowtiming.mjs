...
const CONFIGS = [
  // 기준선은 **백테 현행(익일 시가)** — 검증된 타이밍이다. 여기서의 Δ 로 판정한다.
  { key: 'next', name: 'A 익일시가 (백테 검증본)', file: T, extra: FLOW, grp: 'rule' },
  { key: 'now', name: 'B 당일시가 (라이브 실동작)', file: T, extra: `${FLOW} --flowexitnow`, grp: 'rule' },
  { key: 'off', name: 'C 수급청산 off', file: T, extra: '', grp: 'rule' },
  // 노이즈 바닥: 같은 규칙(A), 데이터만 교란
  { key: 'n1', name: '[바닥] A · 교란 #1', file: 'candles-pert-1.jsonl', extra: FLOW, grp: 'noise' },
  { key: 'n2', name: '[바닥] A · 교란 #2', file: 'candles-pert-2.jsonl', extra: FLOW, grp: 'noise' },
  { key: 'n3', name: '[바닥] A · 교란 #3', file: 'candles-pert-3.jsonl', extra: FLOW, grp: 'noise' },
];
...
const base = R.find(r => r.key === 'next');
...
console.log(`\n=== 판정 ===`);
const now = R.find(r => r.key === 'now'), off = R.find(r => r.key === 'off');
const seedWin = (a, b) => a.cagrs.filter((v, i) => v > (b.cagrs[i] ?? Infinity)).length;
const dNow = now.calmar - base.calmar, dOff = off.calmar - base.calmar;
console.log(`B(라이브 당일시가) vs A(백테 익일시가): ΔCalmar ${dNow >= 0 ? '+' : ''}${dNow.toFixed(2)} · 시드 ${seedWin(now, base)}승 ${SEEDS.length - seedWin(now, base)}패 → ${Math.abs(dNow) > NOISE ? (dNow > 0 ? '실질 개선' : '실질 악화') : '노이즈 내(무차별)'}`);
console.log(`C(off) vs A: ΔCalmar ${dOff >= 0 ? '+' : ''}${dOff.toFixed(2)} · 시드 ${seedWin(off, base)}승 ${SEEDS.length - seedWin(off, base)}패 → ${Math.abs(dOff) > NOISE ? (dOff > 0 ? '규칙이 해롭다' : '규칙이 유효하다') : '노이즈 내(규칙 효과 0)'}`);
console.log('');
if (Math.abs(dNow) <= NOISE) console.log('→ 집행 시점 무차별. **백테와 일치시킨다(A = 종가판정 경로)** — 미검증 괴리를 남길 이유가 없다.');
else if (dNow > 0) console.log('→ 라이브(당일시가)가 실질 우위. 라이브를 두고 **백테를 라이브에 맞춘다**. 단 IS/OOS 분할로 재확인 필요.');
else console.log('→ 라이브(당일시가)가 실질 열위. **라이브를 종가판정 경로로 옮긴다.**');
console.log('※ 어느 결론이든 라이브·백테가 같은 시점을 쓰게 만드는 것이 최소 요건이다.');
