...
const BASE = '--strategies combo-v2 --live-parity --skipneutralrsi --slots 5 --trail 6 --tp1r 1 --tp2r 2 --rsivol 1.25 --no-freshness-check';

const CONFIGS = [
  { key: 'IS-200',  name: 'IS  uni200',  flags: '--to 20240920 --liveuni 200' },
  { key: 'IS-420',  name: 'IS  uni420 (현행)', flags: '--to 20240920 --liveuni 420' },
  { key: 'IS-800',  name: 'IS  uni800',  flags: '--to 20240920 --liveuni 800' },
  { key: 'OS-200',  name: 'OOS uni200',  flags: '--from 20240921 --liveuni 200' },
  { key: 'OS-420',  name: 'OOS uni420 (현행)', flags: '--from 20240921 --liveuni 420' },
  { key: 'OS-800',  name: 'OOS uni800',  flags: '--from 20240921 --liveuni 800' },
];
...
console.log(`=== 유니버스 크기 IS/OOS · ${SEEDS.length}시드 MC (subsample 0.8) · 동시 ${CONC} ===\n`);
