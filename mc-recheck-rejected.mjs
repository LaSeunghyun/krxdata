...
const CONFIGS = [
  // ★ 2026-07-31: 2026-07-30 에 기각한 축들을 **폭락 포함 구간(~20260724)** 으로 재검증한다.
  //   당시 판정은 전부 `--to 20260611`(폭락 없음)이었고, 손절 축에서 그 구간 차이로 순위가 뒤집힌
  //   전례가 있다(06-11 에선 "손절 없음" 통과 → 폭락 넣으면 "넓은 손절 15%"가 최적).
  //   각 축은 당시 **가장 덜 나빴던 변종**만 대표로 올린다(전 변종 재실행은 비용 대비 낮다).
  //   기준선은 현행(손절 15% 기본값 · NEUTRAL 스킵)이다.
  { key: 'base', name: 'off (현행)', file: T, extra: '', grp: 'rule' },
  { key: 'rot', name: '로테이션 보수(0/3)', file: T, extra: '--rotate --rotmaxret 0 --rotminhold 3', grp: 'rule' },
  { key: 'sec', name: '섹터캡 2', file: T, extra: '--sectorcap 2', grp: 'rule' },
  { key: 'dr2', name: 'DOWN rsi2 상한 2', file: T, extra: '--downrsi 2', grp: 'rule' },
  { key: 'mem0', name: 'MA익절 본전초과만', file: T, extra: '--maexitmin 0', grp: 'rule' },
  { key: 'masc', name: 'MA거리 오름차순 정렬', file: T, extra: '--rsimarank asc', grp: 'rule' },
...
console.log(`=== 기각축 폭락구간 재검증 ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
