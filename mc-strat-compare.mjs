/**
 * mc-strat-compare.mjs — 전략 구성 정면비교 10시드 MC (2026-07-29)
 *
...
const CONFIGS = [
  { key: 'IS-live', name: 'IS  combo-v2 라이브(현행)', re: 'combo-v2', flags: `${LIVE} --to 20240920` },
  { key: 'IS-solo', name: 'IS  rsi2 단독(기본)', re: 'rsi2', flags: `${SOLO} --to 20240920` },
  { key: 'IS-solU', name: 'IS  rsi2 단독+uni420', re: 'rsi2', flags: `${SOLO_UNI} --to 20240920` },
  { key: 'OS-live', name: 'OOS combo-v2 라이브(현행)', re: 'combo-v2', flags: `${LIVE} --from 20240921` },
  { key: 'OS-solo', name: 'OOS rsi2 단독(기본)', re: 'rsi2', flags: `${SOLO} --from 20240921` },
  { key: 'OS-solU', name: 'OOS rsi2 단독+uni420', re: 'rsi2', flags: `${SOLO_UNI} --from 20240921` },
];
...
for (const seg of ['IS', 'OS']) {
  const base = R.find(r => r.key === `${seg}-live`);
  if (!base) continue;
  console.log(`\n=== ${seg} 시드별 CAGR — 현행 대비 ===`);
