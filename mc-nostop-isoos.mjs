...
const CONFIGS = [
  // IS/OOS 분할 — 이 세션에서 확인한 프론티어(IS/OOS 순위상관 -1.0) 확인용.
  // NXT 가동이 2025년이므로 IS는 거의 NXT 이전, OOS가 NXT 포함 구간이다.
  { key: 'IS-s7',  name: 'IS  stop 7%(현행)', file: T, extra: '--to 20240920', grp: 'rule' },
  { key: 'IS-s99', name: 'IS  손절 없음', file: T, extra: '--stoppct 99 --to 20240920', grp: 'rule' },
  { key: 'OS-s7',  name: 'OOS stop 7%(현행)', file: T, extra: '--from 20240921', grp: 'rule' },
  { key: 'OS-s99', name: 'OOS 손절 없음', file: T, extra: '--stoppct 99 --from 20240921', grp: 'rule' },
];
...
console.log(`=== 하드손절 IS/OOS 분할 ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
