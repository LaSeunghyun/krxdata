/**
 * mc-strat-compare.mjs — 손절 후 재진입 금지(cooldown) 비교 10시드 MC (2026-07-29)
 *
...
const LIVE = '--strategies combo-v2 --live-parity --skipneutralrsi --slots 5 --trail 6 --tp1r 1 --tp2r 2 --rsivol 1.25 --liveuni 420';
const SOLO = '--strategies rsi2 --slots 5';
const SOLO_UNI = '--strategies rsi2 --slots 5 --rsiuni 420';   // 유니버스만 라이브와 맞춘 변종

const CONFIGS = [
  { key: 'IS-0',  name: 'IS  cooldown 0 (현행)', re: 'combo-v2', flags: LIVE + ' --to 20240920' },
  { key: 'IS-10', name: 'IS  cooldown 10', re: 'combo-v2', flags: LIVE + ' --to 20240920 --cooldown 10' },
  { key: 'IS-15', name: 'IS  cooldown 15', re: 'combo-v2', flags: LIVE + ' --to 20240920 --cooldown 15' },
  { key: 'OS-0',  name: 'OOS cooldown 0 (현행)', re: 'combo-v2', flags: LIVE + ' --from 20240921' },
  { key: 'OS-10', name: 'OOS cooldown 10', re: 'combo-v2', flags: LIVE + ' --from 20240921 --cooldown 10' },
  { key: 'OS-15', name: 'OOS cooldown 15', re: 'combo-v2', flags: LIVE + ' --from 20240921 --cooldown 15' },
];
...
for (const seg of ['IS', 'OS']) {
  const base = R.find(r => r.key === `${seg}-0`);
  if (!base) continue;
  console.log(`\n=== ${seg} 시드별 CAGR — 현행 대비 ===`);
