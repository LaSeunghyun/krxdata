...
```
토스증권 Open API  ──►  daily-ranking.js  ──►  Supabase DB
(폴백: 공공데이터)      (GitHub Actions)        daily_rankings 테이블
OpenDart API      ──►
                                                      │
                                                      ▼
                                          Claude에게 질문
...
| `toss-api.js` | 토스증권 Open API 클라이언트 — OAuth2 토큰 캐시, 10 TPS 페이싱, 현재가/종목정보/일봉 |
...
```env
DART_API_KEY=
PUBLIC_DATA_API_KEY=      # 토스 키 설정 시 생략 가능 (폴백용)
TOSS_CLIENT_ID=           # WTS 설정 > Open API에서 발급 — 가격 소스 1순위
TOSS_CLIENT_SECRET=
SUPABASE_URL=
SUPABASE_KEY=
