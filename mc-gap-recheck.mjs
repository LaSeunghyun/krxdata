/**
 * mc-marank.mjs — MA거리 후보정렬 MC + **노이즈 바닥 동시 측정** (2026-07-30)
 *
 * ═══ 왜 방식을 바꾸나 ═══
...
const ALL_SEEDS = Array.from({ length: 90 }, (_, i) => 101 + i * 37);   // 90시드 결정적 생성(Math.random 불가)
...
const CONFIGS = [
  // ★ 갭정책 재검증. 2026-07-29 채택 근거는 IS Δ0.58 로 당시 노이즈 바닥(0.527) 바로 위였고,
  //   더 중요한 건 검증 구간이 `--to 20260611`(7월 폭락 없음)이었다는 점이다.
  //   갭정책은 **hi120 청산에만 작용**하고 hi120 캡은 UP 6/NEUTRAL 0/DOWN 0 이므로
  //   레짐 UP 구간에서만 효과가 있다 — 표본이 얇을 수 있다는 점을 결과 해석에 반영해야 한다.
  { key: 'g0', name: 'off (갭정책 없음)', file: T, extra: '--gapaxis', grp: 'rule' },
  { key: 'g1', name: '갭정책 trail10 (배포됨)', file: T, extra: '--gapaxis --scenpolicy gap-pol.json', grp: 'rule' },
  { key: 'gt8', name: '갭정책 trail8', file: T, extra: '--gapaxis --scenpolicy gap-pol-t8.json', grp: 'rule' },
  { key: 'gt12', name: '갭정책 trail12', file: T, extra: '--gapaxis --scenpolicy gap-pol-t12.json', grp: 'rule' },
  { key: 'gg1', name: '갭정책 G1만', file: T, extra: '--gapaxis --scenpolicy gap-pol-g1only.json', grp: 'rule' },
  { key: 'n1', name: '[바닥] off · 교란 #1', file: 'candles-pert-1.jsonl', extra: '--gapaxis', grp: 'noise' },
  { key: 'n2', name: '[바닥] off · 교란 #2', file: 'candles-pert-2.jsonl', extra: '--gapaxis', grp: 'noise' },
  { key: 'n3', name: '[바닥] off · 교란 #3', file: 'candles-pert-3.jsonl', extra: '--gapaxis', grp: 'noise' },
];
...
console.log(`=== 갭정책 재검증(7월폭락 포함) ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
...
for (const cfg of CONFIGS) {
  ...
  R.push({ ...cfg, nSeed: rows.length, cagrs: cagr, mdds: mdd, cagrA: cA, cagrM: med(cagr), mddA: mA, calmar: mA > 0 ? cA / mA : 0, win: avg(rows.map(r => r.win)), pf: avg(rows.map(r => r.pf)), trades: avg(rows.map(r => r.n)) });
...
console.log('');
console.log('=== 파산위험 (이 축의 판정 기준) ===');
console.log('구성                          최악시드CAGR  최대MDD  CAGR<0  MDD>40%');
console.log('-'.repeat(76));
for (const r of R) {
  if (r.grp !== 'rule') continue;
  const worst = Math.min(...r.cagrs), maxMdd = Math.max(...r.mdds);
  console.log(`${r.name.padEnd(28)} ${(worst.toFixed(1)+'%').padStart(12)} ${(maxMdd.toFixed(1)+'%').padStart(8)} ${String(r.cagrs.filter(v=>v<0).length+'/'+r.cagrs.length).padStart(7)} ${String(r.mdds.filter(v=>v>40).length+'/'+r.mdds.length).padStart(8)}`);
}
console.log('※ 손절 제거는 평균이 아니라 꼬리로 판단한다. 최악시드가 무너지면 기각.');
console.log(`\n=== 노이즈 바닥 ===`);
...
console.log(`\n=== 판정 ===`);
const rules = R.filter(r => r.grp === 'rule' && r.key !== 'g0');
const real = rules.filter(r => Math.abs(r.calmar - off.calmar) > NOISE);
if (!real.length) console.log(`실질 효과 0건 — 모든 정렬 변종이 노이즈 바닥(${NOISE.toFixed(2)}) 안에 있다.`);
