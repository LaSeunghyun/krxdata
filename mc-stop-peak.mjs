...
const CONFIGS = [
  // ★ 손절 15% 봉우리 확인. 60시드에서 15%가 통과(Δ+0.62 > 바닥 0.316 · 55승5패)했으나
  //   30시드에서 stop20 Δ+0.37 · 손절없음 Δ+0.38 이었어서 15가 최적점인지 미확인이었다.
  //   이웃값이 양쪽으로 떨어지면 봉우리 = 견고, 한쪽이 더 높으면 그쪽으로 옮겨야 한다.
  //   기준선을 15%(현 배포값)로 두고 Δ를 읽는다.
  { key: 's10', name: 'stop 10%', file: T, extra: '--stoppct 10', grp: 'rule' },
  { key: 's12', name: 'stop 12%', file: T, extra: '--stoppct 12', grp: 'rule' },
  { key: 's15', name: 'stop 15% (현 배포)', file: T, extra: '--stoppct 15', grp: 'rule' },
  { key: 's18', name: 'stop 18%', file: T, extra: '--stoppct 18', grp: 'rule' },
  { key: 's20', name: 'stop 20%', file: T, extra: '--stoppct 20', grp: 'rule' },
  { key: 's25', name: 'stop 25%', file: T, extra: '--stoppct 25', grp: 'rule' },
  { key: 'n1', name: '[바닥] stop15 · 교란 #1', file: 'candles-pert-1.jsonl', extra: '--stoppct 15', grp: 'noise' },
  { key: 'n2', name: '[바닥] stop15 · 교란 #2', file: 'candles-pert-2.jsonl', extra: '--stoppct 15', grp: 'noise' },
  { key: 'n3', name: '[바닥] stop15 · 교란 #3', file: 'candles-pert-3.jsonl', extra: '--stoppct 15', grp: 'noise' },
];
...
console.log(`=== 손절 15% 봉우리 확인(7월폭락 포함) ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
...
console.log(`\n=== 판정 ===`);
const rules = R.filter(r => r.grp === 'rule' && r.key !== 's15');
