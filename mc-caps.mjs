/**
 * mc-strat-compare.mjs — CAPS 프리셋 비교 10시드 MC (2026-07-29)
 *
...
const CONFIGS = [
  { key: 'IS-A', name: 'IS  caps A (현행)', re: 'combo-v2', flags: LIVE + ' --to 20240920 --caps A' },
  { key: 'IS-B', name: 'IS  caps B', re: 'combo-v2', flags: LIVE + ' --to 20240920 --caps B' },
  { key: 'IS-C', name: 'IS  caps C', re: 'combo-v2', flags: LIVE + ' --to 20240920 --caps C' },
  { key: 'OS-A', name: 'OOS caps A (현행)', re: 'combo-v2', flags: LIVE + ' --from 20240921 --caps A' },
  { key: 'OS-B', name: 'OOS caps B', re: 'combo-v2', flags: LIVE + ' --from 20240921 --caps B' },
  { key: 'OS-C', name: 'OOS caps C', re: 'combo-v2', flags: LIVE + ' --from 20240921 --caps C' },
];
...
for (const seg of ['IS', 'OS']) {
  const base = R.find(r => r.key === `${seg}-A`);
  if (!base) continue;
  console.log(`\n=== ${seg} 시드별 CAGR — 현행 대비 ===`);
