/**
 * mc-maexitmin.mjs — MA익절 최소수익 조건 MC + **노이즈 바닥 동시 측정** (2026-07-30)
 *
 * ═══ 왜 방식을 바꾸나 ═══
...
const CONFIGS = [
  // ── 규칙 축: MA 익절에 '종가 > 진입가×(1+N%)' AND 조건 ──
  { key: 'base', name: 'off (현행 · 손실에도 MA익절)', file: T, extra: '', grp: 'rule' },
  { key: 'm-1', name: 'maexitmin -1 (-1%까지 허용)', file: T, extra: '--maexitmin -1', grp: 'rule' },
  { key: 'm0', name: 'maexitmin 0 (본전 초과만)', file: T, extra: '--maexitmin 0', grp: 'rule' },
  { key: 'm1', name: 'maexitmin 1 (+1% 초과만)', file: T, extra: '--maexitmin 1', grp: 'rule' },
  { key: 'm2', name: 'maexitmin 2 (+2% 초과만)', file: T, extra: '--maexitmin 2', grp: 'rule' },
  // ── 노이즈 바닥 축 ──
...
console.log(`=== MA익절 최소수익 조건 ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
...
if (!real.length) console.log(`실질 효과 0건 — 모든 maexitmin 변종이 노이즈 바닥(${NOISE.toFixed(2)}) 안에 있다.`);
