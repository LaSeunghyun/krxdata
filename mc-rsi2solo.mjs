...
const CONFIGS = [
  { key: 'IS-cv2', name: 'IS  combo-v2 라이브(현행)', flags: '--to 20240920' },
  { key: 'IS-r',   name: 'IS  rsi2 단독(기본파라미터)', flags: '--to 20240920 --strategies rsi2' },
  { key: 'OS-cv2', name: 'OOS combo-v2 라이브(현행)', flags: '--from 20240921' },
  { key: 'OS-r',   name: 'OOS rsi2 단독(기본파라미터)', flags: '--from 20240921 --strategies rsi2' },
];

const RE = /(?:combo-v2|rsi2)\s+(\d+)\s+(\d+)%\s+([\d.]+)\s+([\d.-]+)%\s+([\d.-]+)%\s+(\d+)%\s+([\d.]+)일/;
...
console.log(`=== rsi2 단독 vs combo-v2 라이브 IS/OOS · ${SEEDS.length}시드 MC (subsample 0.8) · 동시 ${CONC} ===\n`);
