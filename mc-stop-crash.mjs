...
const CONFIGS = [
  // ★ 2026-07-30 재실행: 기간을 **7월 폭락 포함(--to 20260724)** 으로 늘렸다.
  //   이유: 기존 모든 검증이 --to 20260611 이었고, 07-24까지 늘리면 기준선이 무너진다 —
  //     CAGR 26.2%→17.3% · MDD 25.7%→**43.3%**. 즉 현 국면의 실제 MDD 는 26% 가 아니라 43% 다.
  //   손절 축은 폭락 구간이 없으면 검정이 성립하지 않는다(손절은 폭락에서 작동하는 장치다).
  //   단일경로(07-24까지): stop7 17.3%/43.3% · stop10 13.8%/45.4% · stop15 32.8%/29.3% · 없음 29.8%/33.0%
  //   → "없음"보다 "넓은 손절"이 나을 가능성. 비단조적이라 MC 필요.
  { key: 's7', name: 'stop 7% (현행)', file: T, extra: '', grp: 'rule' },
  { key: 's10', name: 'stop 10%', file: T, extra: '--stoppct 10', grp: 'rule' },
  { key: 's15', name: 'stop 15%', file: T, extra: '--stoppct 15', grp: 'rule' },
  { key: 's20', name: 'stop 20%', file: T, extra: '--stoppct 20', grp: 'rule' },
  { key: 's99', name: '손절 없음(99%)', file: T, extra: '--stoppct 99', grp: 'rule' },
...
console.log(`=== 하드손절 폭(7월폭락 포함) ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
