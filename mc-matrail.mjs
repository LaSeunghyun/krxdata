/**
 * mc-matrail.mjs — rsi2 트레일 라이브-백테 괴리 판정 (2026-07-29)
 *
 ...
 *
 * 실행: node mc-matrail.mjs [--seeds 10] [--conc 4]
 */
...
const CONFIGS = [
  { key: 'A',    name: 'A 기준 (고정 트레일 6%)',        flags: '' },
  { key: 'M3',   name: 'M3 hi120 추세선 MA3 이탈',       flags: '--matrail 3' },
  { key: 'M5',   name: 'M5 hi120 추세선 MA5 이탈',       flags: '--matrail 5' },
  { key: 'M7',   name: 'M7 hi120 추세선 MA7 이탈',       flags: '--matrail 7' },
  { key: 'M10',  name: 'M10 hi120 추세선 MA10 이탈',     flags: '--matrail 10' },
  { key: 'LS10', name: 'LS10 rsi2 직전10일 저가 이탈손절', flags: '--rsilowstop 10' },
];
...
console.log(`=== rsi2 트레일 괴리 판정 · ${SEEDS.length}시드 MC (subsample 0.8) · 동시 ${CONC} ===\n`);
...
const base = results.find(r => r.key === 'A');
console.log('\n=== 결과 ===');
...
if (base) {
  ...
  console.log('\n※ MA5가 봉우리라면 M3 < M5 > M7 > M10 이어야 한다. 계속 짧아질수록 좋으면 끝값 편향이다.');
