/**
 * mc-downrsi.mjs — DOWN레짐 rsi2 상한 MC + **노이즈 바닥 동시 측정** (2026-07-30)
 *
 * ═══ 왜 방식을 바꾸나 ═══
...
const CONFIGS = [
  // ── 규칙 축: DOWN 레짐 rsi2 슬롯 상한 ──
  //   downrsi 4 는 현행 DOWN 캡과 같은 값이므로 **항등 대조군**이다(배선 무해성 확인용).
  { key: 'base', name: 'off (현행 DOWN rsi2=4)', file: T, extra: '', grp: 'rule' },
  { key: 'd4', name: 'downrsi 4 (항등 대조)', file: T, extra: '--downrsi 4', grp: 'rule' },
  { key: 'd3', name: 'downrsi 3', file: T, extra: '--downrsi 3', grp: 'rule' },
  { key: 'd2', name: 'downrsi 2', file: T, extra: '--downrsi 2', grp: 'rule' },
  { key: 'd1', name: 'downrsi 1', file: T, extra: '--downrsi 1', grp: 'rule' },
  { key: 'd0', name: 'downrsi 0 (DOWN rsi2 전면차단)', file: T, extra: '--downrsi 0', grp: 'rule' },
...
console.log(`=== DOWN레짐 rsi2 상한 ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
...
if (!real.length) console.log(`실질 효과 0건 — 모든 downrsi 변종이 노이즈 바닥(${NOISE.toFixed(2)}) 안에 있다.`);
