...
const CONFIGS = [
  // ★ 2026-07-31 재실행: 첫 실행은 **내 실수로 무효**였다 — MC(conc3)와 별도 연구 스크립트
  //   2개를 각각 6GB 힙으로 동시에 돌려 9구성 중 5개가 0시드로 죽었다(노이즈 바닥 1.575 = 쓰레기값).
  //   단독 실행 + conc 2 로 재측정한다. 구성도 7개로 줄였다(ATR>14 제외 — 배제폭이 너무 좁아 정보 적음).
  { key: 'off', name: 'off (배제 없음, 현행)', file: T, extra: '', grp: 'rule' },
  { key: 'a5', name: 'ATR>5% 배제 (구 시도·p50)', file: T, extra: '--rsiatrmax 5', grp: 'rule' },
  { key: 'a9', name: 'ATR>9% 배제 (~p90) ★후보', file: T, extra: '--rsiatrmax 9', grp: 'rule' },
  { key: 'a11', name: 'ATR>11% 배제 (~p95)', file: T, extra: '--rsiatrmax 11', grp: 'rule' },
  { key: 'n1', name: '[바닥] off · 교란 #1', file: 'candles-pert-1.jsonl', extra: '', grp: 'noise' },
  { key: 'n2', name: '[바닥] off · 교란 #2', file: 'candles-pert-2.jsonl', extra: '', grp: 'noise' },
  { key: 'n3', name: '[바닥] off · 교란 #3', file: 'candles-pert-3.jsonl', extra: '', grp: 'noise' },
];
...
console.log(`=== 고ATR 배제 임계 스윕(7월폭락 포함) ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
