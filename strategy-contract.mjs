...
export const AI_TRADER = Object.freeze({
   ...
   * ★ 180_000 → 300_000. 이 타이머는 **spawn 시점부터** 돌고 spawn 대상은 `flock claude` 라
   *   **락 대기 시간이 포함된다.** 따라서 timeoutMs > lockWaitSec + claude 실행시간 이어야 한다.
   *   lockWaitSec 를 150 으로 올리면서 180s 로 두면 락을 늦게 잡은 **정상 호출이 SIGKILL** 되고
   *   락 경합이 timedOut(=실패)으로 오분류돼 failStreak 가 오른다. 150 + 150 으로 맞춘다.
   */
  timeoutMs: 300_000,
  /**
   ...
   *
   * ★ 20 → 150. 20 은 **락 보유자의 실제 점유시간과 맞지 않는 값**이었다.
   *   telegram-agent 의 claude 타임아웃은 300s 이고(2026-07-24 에 180→300 으로 올린 사유가
   *   "개별종목 분석이 종종 180s 넘김, 재현확인됨"이다), watchdog 진단은 240s 다.
   *   20s 대기로는 짧은 텔레그램 턴(실측 51s)조차 못 이겨 사실상 **항상 양보**한다.
   *   대기를 늘려도 안전한 근거: consultTrader 는 항상 즉시 반환하고 claude 호출은 백그라운드라
   *   30초 청산 루프를 막지 않는다(이 파일의 계약이 아니라 ai-trader.mjs 의 구조다).
   *   즉 "기다렸다 판단을 받는 것"이 "일찍 포기하고 다음 gap 까지 노는 것"보다 언제나 낫다.
   *   상한 근거는 staleFallbackMin(10분) 예산 — 150s 면 양보해도 그 안에 재시도가 들어간다.
   *   ※ 이 값을 올릴 때는 timeoutMs 를 반드시 같이 올린다(타이머가 락 대기를 포함한다).
   */
  lockWaitSec: 150,
  /**
