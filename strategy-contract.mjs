...
export const AI_TRADER = Object.freeze({
  ...
  failOpen: false,    // false=연속실패 시 그날 매수 중단(보수) / true=기계 로직 폴백
  failOpenAfter: 3,
  model: 'sonnet',    // claude CLI 모델 별칭 → 실측 claude-sonnet-5 (VM claude 2.1.216)
  // ── 운영 안전 (VM RAM 956MB · available ~374MB · claude 1프로세스 ~300-400MB) ──
  minMemMb: 450,      // MemAvailable 이 이 값 미만이면 호출 스킵(OOM 방어)
  ...
  rotate: Object.freeze({
    enabled: true,
    maxPerDay: 2,        // 하루 즉시교체 상한. 왕복비용 0.33%p × 이 값이 확정 마찰비용
    minHoldDays: 1,      // 진입 당일 교체 금지(휩소 방어 — 07-28~29 두산퓨얼셀 4회 재진입 전례)
    maxSellLossPct: 8,   // 이 이상 손실인 종목은 즉시교체로 팔지 않는다(손절 규칙에 맡긴다)
  }),
});
