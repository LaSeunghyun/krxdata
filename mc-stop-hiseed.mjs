...
const CONFIGS = [
  // ★ 손절 15% 고시드 확정. 30시드에서 Δ+0.44 < 바닥 0.486 으로 판정 불가였다.
  //   시드를 늘리면 바닥이 좁아진다(실측: 10시드 0.527 → 30시드 0.486/0.268). 60~90시드로 확정한다.
  { key: 's7', name: 'stop 7% (구 현행)', file: T, extra: '--stoppct 7', grp: 'rule' },
  { key: 's15', name: 'stop 15% (신 현행)', file: T, extra: '--stoppct 15', grp: 'rule' },
  { key: 'n1', name: '[바닥] stop7 · 교란 #1', file: 'candles-pert-1.jsonl', extra: '--stoppct 7', grp: 'noise' },
  { key: 'n2', name: '[바닥] stop7 · 교란 #2', file: 'candles-pert-2.jsonl', extra: '--stoppct 7', grp: 'noise' },
  { key: 'n3', name: '[바닥] stop7 · 교란 #3', file: 'candles-pert-3.jsonl', extra: '--stoppct 7', grp: 'noise' },
  { key: 'n4', name: '[바닥] stop7 · 교란 #4', file: 'candles-pert-4.jsonl', extra: '--stoppct 7', grp: 'noise' },
];
...
console.log(`=== 손절 15% 고시드 확정(7월폭락 포함) ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
...
console.log(`\n=== 판정 ===`);
const rules = R.filter(r => r.grp === 'rule' && r.key !== 's7');
