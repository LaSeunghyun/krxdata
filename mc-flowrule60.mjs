...
const CONFIGS = [
  { key: 'on', name: 'B 수급청산 ON (라이브 현행)', file: T, extra: FLOW, grp: 'rule' },
  { key: 'off', name: 'C 수급청산 OFF', file: T, extra: '', grp: 'rule' },
  { key: 'n1', name: '[바닥] ON · 교란 #1', file: 'candles-pert-1.jsonl', extra: FLOW, grp: 'noise' },
  { key: 'n2', name: '[바닥] ON · 교란 #2', file: 'candles-pert-2.jsonl', extra: FLOW, grp: 'noise' },
  { key: 'n3', name: '[바닥] ON · 교란 #3', file: 'candles-pert-3.jsonl', extra: FLOW, grp: 'noise' },
];
...
const dOff = off.calmar - base.calmar;
const onWin = seedWin(base, off);
console.log(`ON(현행) vs OFF: ΔCalmar ${(-dOff >= 0 ? '+' : '')}${(-dOff).toFixed(2)} (ON 기준) · 시드 ON ${onWin}승 ${SEEDS.length - onWin}패`);
console.log(`노이즈 바닥 ${NOISE.toFixed(3)} · |Δ| ${Math.abs(dOff).toFixed(3)} → ${Math.abs(dOff) > NOISE ? '판정 가능' : '여전히 경계·판정 불가'}`);
console.log('');
if (Math.abs(dOff) <= NOISE) {
  console.log('→ 60시드에서도 바닥 안. **규칙 효과가 확립되지 않는다.**');
  console.log('   이 경우 판단 기준은 꼬리다 — 최악시드·최대MDD 가 나은 쪽을 택한다(방어 규칙의 존재 이유가 꼬리이므로).');
} else if (dOff > 0) {
  console.log('→ OFF 가 실질 우위. **수급청산을 끈다**(FLOW_EXIT.enabled = false).');
} else {
  console.log('→ ON 이 실질 우위. **수급청산을 현행 유지**한다.');
}
console.log('※ 이 판정은 라이브 타이밍(당일 시가)으로 잰 것이다 — 원래 채택 근거(익일 시가)와 다른 기준이다.');
