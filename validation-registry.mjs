 ...
 * 게다가 `--to 20260611` 은 방법론 §1-B 가 "가장 큰 함정"으로 지목한 **폭락 제외 구간**이라
 * 방어장치 검정이 성립하지 않는다.
 *
 * ※ 영향 범위 정정(2026-08-22 VM 실측): **일일 크론은 영향받지 않았다.** 크론은
 *   `node validate-hypotheses.mjs --data-only` 라 백테 가설을 통째로 건너뛰고 데이터가설 3건
 *   (forecast_edge·live_track·flow_edge)만 돈다. 낡은 base 와 어긋난 myVerdict 는 **수동 full
 *   실행에서만** 드러나는 휴면 결함이었다. 그래도 고치는 이유는 그 수동 실행이 판정의 근거가 되고,
 *   FLIP 경보가 처음 울릴 때 신뢰를 잃기 때문이다.
 *
