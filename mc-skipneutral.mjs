...
const LIVE = '--strategies combo-v2 --live-parity --slots 5 --trail 6 --tp1r 1 --tp2r 2 --liveuni 420 --rsivol 0';   // ★ --skipneutralrsi 를 뺐다 — arm 별로 붙인다
const T = 'candles-daily-toss-clean.jsonl';

const CONFIGS = [
  // ★ skipneutralrsi 재검증. 2026-07-22 채택 근거가 **MC 6시드 + 폭락 없는 구간**이었다.
  //   오늘 손절 축에서 확인한 대로 구간에 7월 폭락을 넣으면 판정이 뒤집힐 수 있다.
  //   지금 라이브 레짐이 실제로 NEUTRAL 이라(07-31) 결과가 즉시 실용적이다:
  //   NEUTRAL 에서는 hi120 캡 0 + rsi2 skipNeutral 로 후보가 0건이 되어 현금 430만·슬롯 1개가 놀고 있다.
  { key: 'skip', name: 'NEUTRAL rsi2 스킵 (현행)', file: T, extra: '--skipneutralrsi', grp: 'rule' },
  { key: 'noskip', name: 'NEUTRAL rsi2 허용', file: T, extra: '', grp: 'rule' },
  { key: 'n1', name: '[바닥] 현행 · 교란 #1', file: 'candles-pert-1.jsonl', extra: '--skipneutralrsi', grp: 'noise' },
  { key: 'n2', name: '[바닥] 현행 · 교란 #2', file: 'candles-pert-2.jsonl', extra: '--skipneutralrsi', grp: 'noise' },
  { key: 'n3', name: '[바닥] 현행 · 교란 #3', file: 'candles-pert-3.jsonl', extra: '--skipneutralrsi', grp: 'noise' },
  { key: 'n4', name: '[바닥] 현행 · 교란 #4', file: 'candles-pert-4.jsonl', extra: '--skipneutralrsi', grp: 'noise' },
];
...
console.log(`=== NEUTRAL rsi2 스킵 재검증(7월폭락 포함) ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
...
console.log(`\n=== 판정 ===`);
const rules = R.filter(r => r.grp === 'rule' && r.key !== 'skip');
