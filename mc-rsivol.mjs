/**
 * mc-strat-compare.mjs — rsivol 0 vs 1.25 (거래량 확인 필터) 10시드 MC (2026-07-29)
 *
...
const CONFIGS = [
  { key: 'IS-0',   name: 'IS  rsivol 0 (필터 제거)', re: 'combo-v2', flags: LIVE + ' --to 20240920 --rsivol 0' },
  { key: 'IS-125', name: 'IS  rsivol 1.25 (현행)', re: 'combo-v2', flags: LIVE + ' --to 20240920 --rsivol 1.25' },
  { key: 'OS-0',   name: 'OOS rsivol 0 (필터 제거)', re: 'combo-v2', flags: LIVE + ' --from 20240921 --rsivol 0' },
  { key: 'OS-125', name: 'OOS rsivol 1.25 (현행)', re: 'combo-v2', flags: LIVE + ' --from 20240921 --rsivol 1.25' },
];
...
for (const seg of ['IS', 'OS']) {
  const base = R.find(r => r.key === `${seg}-125`);
  if (!base) continue;
  console.log(`\n=== ${seg} 시드별 CAGR — 현행 대비 ===`);
