...
const CONFIGS = [
  // ── 규칙 축: 확신도 동률 내 MA거리 정렬 (풀 불변, 순서만 바꾼다) ──
  { key: 'base', name: 'off (임의 순서 = 현행)', file: T, extra: '', grp: 'rule' },
  { key: 'asc', name: 'MA거리 작은 것 우선(카카오형)', file: T, extra: '--rsimarank asc', grp: 'rule' },
  { key: 'desc', name: 'MA거리 큰 것 우선(하이브형)', file: T, extra: '--rsimarank desc', grp: 'rule' },
  // ── 노이즈 바닥 축 ──
...
console.log(`=== MA거리 후보정렬 ${SEEDS.length}시드 MC + 노이즈 바닥 동시측정 ===`);
