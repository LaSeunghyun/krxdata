...
const CONFIGS = [
  { key: 'IS-p1', name: 'IS  gap-pol (배포됨)', re: 'combo-v2', flags: LIVE + ' --rsivol 0 --gapaxis --to 20240920 --scenpolicy gap-pol.json' },
  { key: 'IS-s5', name: 'IS  +G1 rsi2손절5', re: 'combo-v2', flags: LIVE + ' --rsivol 0 --gapaxis --to 20240920 --scenpolicy gap-rsi-s5.json' },
  { key: 'IS-s4', name: 'IS  +G1 rsi2손절4', re: 'combo-v2', flags: LIVE + ' --rsivol 0 --gapaxis --to 20240920 --scenpolicy gap-rsi-s4.json' },
  { key: 'OS-p1', name: 'OOS gap-pol (배포됨)', re: 'combo-v2', flags: LIVE + ' --rsivol 0 --gapaxis --from 20240921 --scenpolicy gap-pol.json' },
  { key: 'OS-s5', name: 'OOS +G1 rsi2손절5', re: 'combo-v2', flags: LIVE + ' --rsivol 0 --gapaxis --from 20240921 --scenpolicy gap-rsi-s5.json' },
  { key: 'OS-s4', name: 'OOS +G1 rsi2손절4', re: 'combo-v2', flags: LIVE + ' --rsivol 0 --gapaxis --from 20240921 --scenpolicy gap-rsi-s4.json' },
];
...
for (const seg of ['IS', 'OS']) {
  const base = R.find(r => r.key === `${seg}-p1`);
  if (!base) continue;
  console.log(`\n=== ${seg} 시드별 CAGR — 현행 대비 ===`);
